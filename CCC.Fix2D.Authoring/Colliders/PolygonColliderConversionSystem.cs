...
namespace CCC.Fix2D.Authoring
{
    ...
    internal sealed class PolygonColliderConversionSystem : GameObjectConversionSystem
    {
        ...
        protected override void OnUpdate()
        {
            Entities.ForEach((UnityEngine.PolygonCollider2D collider) =>
            {
                ...
                if (ConversionUtilities.CanConvertCollider(collider))
                {
                    try
                    {
                        ...
                        if (collider.pathCount != 1 || colliderPointCount > Collider.PolygonConstants.MaxVertexCount)
                            return;
            ...
            {
                // Convert the collider if it's valid.
                if (ConversionUtilities.CanConvertCollider(collider) && collider.Shape == PhysicsBodyAuth.ColliderShape.Polygon)
                {
                    try
                    {
                        // Only single paths with no more than the maximum allowed vertex allowed.
                        // NOTE: Until we implement a convex polygon decomposition, only the convex hull of these points will be used.
                        var colliderPointCount = collider.PolygonPoints.Count;
                        if (/*collider.pathCount != 1 || */colliderPointCount > Collider.PolygonConstants.MaxVertexCount)
                            throw new ArgumentException($"Polygon cannot have more than {Collider.PolygonConstants.MaxVertexCount} vertices.", "Polygon Vertices.");
                        ...
                        for (var i = 0; i < colliderPointCount; ++i)
                        {
                            var vertex = localToWorld.MultiplyPoint(offset + (Vector3)collider.PolygonPoints[i]);
                            vertices[i] = new float2(vertex.x, vertex.y);
                        }
