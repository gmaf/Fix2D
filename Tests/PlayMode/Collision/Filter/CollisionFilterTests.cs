...
namespace CCC.Fix2D.Tests
{
    class CollisionFilterTests
    {
        ...
        [Test]
        public void CollisionFilterTest_EmptyState()
        {
            Assert.IsTrue(default(CollisionFilter).IsEmpty);
            Assert.IsTrue(new CollisionFilter { BelongsTo = 0, CollidesWith = 1 }.IsEmpty);
            Assert.IsTrue(new CollisionFilter { BelongsTo = 1, CollidesWith = 0 }.IsEmpty);
        }
    }
}
