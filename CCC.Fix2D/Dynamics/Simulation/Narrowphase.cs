                ...
                for (int i = 0; i < numPairsToRead; i++)
                {
                    ...
                    if (dispatchPair.IsValid)
                    {
                        if (dispatchPair.IsContact)
                        {
                            ...
                            var pair = new PhysicsBody.IndexPair
                            {
                                PhysicsBodyIndexA = dispatchPair.PhysicsBodyIndexA,
                                PhysicsBodyIndexB = dispatchPair.PhysicsBodyIndexB
                            };

                            PhysicsBody rigidBodyA = world.AllBodies[pair.PhysicsBodyIndexA];
                            PhysicsBody rigidBodyB = world.AllBodies[pair.PhysicsBodyIndexB];

                            PhysicsBody.MotionVelocity motionVelocityA = pair.PhysicsBodyIndexA < world.BodyMotionVelocity.Length ?
                                world.BodyMotionVelocity[pair.PhysicsBodyIndexA] : PhysicsBody.MotionVelocity.Zero;
                            PhysicsBody.MotionVelocity motionVelocityB = pair.PhysicsBodyIndexB < world.BodyMotionVelocity.Length ?
                                world.BodyMotionVelocity[pair.PhysicsBodyIndexB] : PhysicsBody.MotionVelocity.Zero;
