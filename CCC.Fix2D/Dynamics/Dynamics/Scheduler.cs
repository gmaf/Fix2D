...
namespace CCC.Fix2D
{
    ...
    public class DispatchPairSequencer : IDisposable
    {
        ...
        //
        // We additionally choose indices so that PhysicsBodyIndexA < PhysicsBodyIndexB. This has subtle side-effects:
        ...
        //
        [DebuggerDisplay("{IsContact ? \"Contact\" : \"Joint\"}, [{PhysicsBodyIndexA}, {PhysicsBodyIndexB}]")]
        public struct DispatchPair
        {
            ...
            public static DispatchPair CreateCollisionPair(PhysicsBody.IndexPair pair)
            {
            ...
            public static DispatchPair CreateJoint(PhysicsBody.IndexPair pair, int jointIndex, int allowCollision)
            {
                Assert.IsTrue(jointIndex < k_InvalidJointIndex);
                return Create(pair, jointIndex, allowCollision);
            }

            private static DispatchPair Create(PhysicsBody.IndexPair pair, int jointIndex, int allowCollision)
            {
                Assert.IsTrue(pair.PhysicsBodyIndexA < k_InvalidBodyIndex && pair.PhysicsBodyIndexB < k_InvalidBodyIndex);
                int selectedA = math.min(pair.PhysicsBodyIndexA, pair.PhysicsBodyIndexB);
                int selectedB = math.max(pair.PhysicsBodyIndexA, pair.PhysicsBodyIndexB);
            ...
            int threadCountHint = 0)
        {
            ...
            if (threadCountHint <= 0)
            {
                ...
                returnHandles.FinalExecutionHandle = new CreateDispatchPairsJobST
                {
                    ...
                    Joints = world.Joints,
                    NumDynamicBodies = world.DynamicBodyCount
                }.Schedule(inputDeps);
            ...
            JobHandle sortHandle;
            {
                ...
                sortHandle = ScheduleSortJob(world.BodyCount,
        ...
        // with DispatchPair.PhysicsBodyIndexA, resulting in sub arrays with the same bodyA index.
        ...
        private struct CreateUnsortedDispatchPairsJob : IJob
        {
                ...
                NativeList<DispatchPair> unsortedDispatchPairs, NativeList<DispatchPair> dispatchPairsUninitialized)
            {
                ...
                for (int i = 0; i < dynamicVsDynamicPairReader.ForEachCount; i++)
                {
                    ...
                    {
                        PhysicsBody.IndexPair pair = dynamicVsDynamicPairReader.Read<PhysicsBody.IndexPair>();
                ...
                for (int i = 0; i < staticVsDynamicPairReader.ForEachCount; i++)
                {
                    ...
                    for (int j = 0; j < rangeItemCount; j++)
                    {
                        PhysicsBody.IndexPair pair = staticVsDynamicPairReader.Read<PhysicsBody.IndexPair>();
                        pairs[counter++] = DispatchPair.CreateCollisionPair(pair);
                    }
        ...
        unsafe internal struct RadixSortPerBodyAJob : IJob
        {
            ...
            // associated with PhysicsBodyIndexA in DispatchPair.
        ...
        internal struct CreateDispatchPairPhasesJob : IJob
        {
                ...
                ref NativeArray<SolverSchedulerInfo.SolvePhaseInfo> phaseInfo, bool useTables = true)
            {
                ...
                for (int i = 0; i < dispatchPairs.Length; i++)
                {
                    int bodyIndexA = dispatchPairs[i].PhysicsBodyIndexA;
                    int bodyIndexB = dispatchPairs[i].PhysicsBodyIndexB;
                ...
                {
                    int bodyIndexB = dispatchPairs[pairIndex].PhysicsBodyIndexB;

                    bool isBodyBStatic = bodyIndexB >= numDynamicBodies;
                    if (isBodyBStatic && phaseIdPerPair[pairIndex] == k_unitializedPair)
                    {
                        int bodyIndexA = dispatchPairs[pairIndex].PhysicsBodyIndexA;
                    ...
                    var firstWorkItemIndex = info.FirstWorkItemIndex;
                    {
                        for (int j = firstWorkItemIndex; j < firstWorkItemIndex + info.NumWorkItems - 1; j++)
                        {
                            for (int k = j + 1; k < j + 1 + info.NumWorkItems; k++)
                            {
                                ...
                                {
                                    int aIndex1 = phasedDispatchPairs[pairIndex1].PhysicsBodyIndexA;
                                    int bIndex1 = phasedDispatchPairs[pairIndex1].PhysicsBodyIndexB;
                                    for (int pairIndex2 = secondIndex; pairIndex2 < math.min(secondIndex + info.BatchSize, dispatchPairCount); pairIndex2++)
                                    {
                                        int aIndex2 = phasedDispatchPairs[pairIndex2].PhysicsBodyIndexA;
                                        int bIndex2 = phasedDispatchPairs[pairIndex2].PhysicsBodyIndexB;
